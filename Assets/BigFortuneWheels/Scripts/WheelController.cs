    ...
    public class WheelController : MonoBehaviour
    {
        ...
        private async void CollectCoins(int amount, string spinWheelId, bool isBigWin)
        {
            ...
            else
            {
                ...
                if (LoaderManager.instance.SpinState == LoaderManager.SpinWheelState.GameEndSpin)
                {
                    ...
                    //        _button_2X.onClick.AddListener(() => { /*_collectButton.interactable = false;*/IronSourceAdsController.Instance.ShowRewardedAd(StringConstants.Double_Coin_Reward_PlacementName); });
                    //        IronSourceAdsController.Instance.OnRewardAdClosedEvent.RemoveAllListeners();
                    //        IronSourceAdsController.Instance.OnRewardAdClosedEvent.AddListener(() =>
                    //        {
                    //            CollectSpinWheelReward(spinWheelId, true, true, () => { /*NativeFunction.UpdateCoins(amount);*/PlayerPrefs.SetInt(StringConstants.Pp_SpinWinCoins, amount * 2); AddCoins(amount * 2, isBigWin); LoaderManager.instance.DisableLoader(); });
                    //            //UnityMainThreadDispatcher.Instance.Enqueue(() =>
                    //            //{
                    //            //});
                    //        });
                    //        IronSourceAdsController.Instance.OnRewardAdFailedEvent.RemoveAllListeners();
                    //        IronSourceAdsController.Instance.OnRewardAdFailedEvent.AddListener(() =>
                    //        {
                    //            CollectSpinWheelReward(spinWheelId, true, true, () => { /*NativeFunction.UpdateCoins(amount);*/PlayerPrefs.SetInt(StringConstants.Pp_SpinWinCoins, amount * 2); AddCoins(amount * 2, isBigWin); LoaderManager.instance.DisableLoader(); });
                    //            //UnityMainThreadDispatcher.Instance.Enqueue(() =>
                    //            //{
        ...
        {
            //if (LoaderManager.instance.SpinState == LoaderManager.SpinWheelState.Rewarded)
            //{
            //    int temp = PlayerPrefs.GetInt(StringConstants.Pp_Reward);
            //    temp -= 1;
            //    PlayerPrefs.SetInt(StringConstants.Pp_Reward, temp);

            //    string lastSpinReward = DateTime.Now.ToString();
            //    PlayerPrefs.SetString(StringConstants.Pp_LastRewardSpinTime, lastSpinReward);
            //    PlayerPrefs.Save();
            //    Utility.myLog("Reward Value ->" + temp);
            //}
            //if (LoaderManager.instance.SpinState == LoaderManager.SpinWheelState.LuckySpin)
            //{
            //    int temp = PlayerPrefs.GetInt(StringConstants.Pp_LuckySpin);
            //    temp -= 1;
            //    PlayerPrefs.SetInt(StringConstants.Pp_LuckySpin, temp);

            //    string lastLuckySpin = DateTime.Now.ToString();
            //    PlayerPrefs.SetString(StringConstants.Pp_LastLuckyRewardTime, lastLuckySpin);

            //    Utility.myLog("Luckyspin Value->" + temp);
            //}
            ...
            if (data != null)
            {
                if (data.spinWheelId != null) // old (data.statuscode == 200)
                {
                    ...
                    //In Case of GameEnd Spin and want to show Score to user.
                    //if (GamesAssistant.gameID == 7)
                    //{
